<script>
  ...
  async function handleFindUser(e) {
    ...
    try {
      ...
      // Handle both snake_case (from proto) and camelCase response formats
      const token = response.reset_token || response.resetToken;
      
      if (response.success && token) {
        userFound = true;
        resetToken = token;
        toastStore.success('User found! Please enter your new password.');
      } else if (response.success && !token) {
        // User not found (backend returns success for security but no token)
        error = 'No account found with this email or phone number.';
      } else {
        error = response.message || 'Failed to verify account';
      }
    } catch (e) {
      error = e.message || 'Failed to verify account';
    } finally {
      loading = false;
    }
  }
    ...
    <div class="bg-white rounded-xl border border-[var(--color-border)] p-6">
      ...
      {:else if userFound}
        ...
        <div>
          <button 
            onclick={goBack}
            class="flex items-center gap-1 text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text)] mb-4"
          >
          ...
          <form onsubmit={handleResetPassword} class="space-y-4">
            <!-- Hidden Reset Token -->
            <input type="hidden" bind:value={resetToken} />
      ...
      {:else}
        ...
        <form onsubmit={handleFindUser} class="space-y-4">
          <!-- Email or Phone -->
          <div>
